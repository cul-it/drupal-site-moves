...
#  --dry-run to test
#  -no-perms so we don't have to use sudo rsync
...
if [ "$LOCAL_IS_PRODUCTION_SERVER" -eq 1 ] ;then
  ...
  rsync -avcz -e "ssh -l $REMOTE_USER" --omit-dir-times -no-perms --chmod=ug=rwX \
    --exclude=sites/*/settings.php \
    --exclude=sites/all/* \
  ...
  rsync -avcz -e "ssh -l $REMOTE_USER" --delete --omit-dir-times -no-perms --chmod=ug=rwX \
    --exclude=sites/*/settings.php \
