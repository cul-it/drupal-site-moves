...
sudo chown -R "$LOCAL_USER:$LOCAL_USER_GROUP" "${LOCAL_SITE_MOVES_BACKUP_PATH}" || error_exit "can't chown ${LOCAL_SITE_MOVES_BACKUP_PATH}"
sudo chmod -R ug=rwX,o=rX "${LOCAL_SITE_MOVES_BACKUP_PATH}" || error_exit "can't chmod ${LOCAL_SITE_MOVES_BACKUP_PATH}"
...
#  --dry-run to test
#  --no-perms --no-times --no-group so we don't have to use sudo rsync
...
if [ "$LOCAL_IS_PRODUCTION_SERVER" -eq 1 ] ;then
  ...
  rsync -avcz -e "ssh -l $REMOTE_USER" --omit-dir-times --no-perms --no-times --no-group --chmod=ug=rwX \
    --exclude=sites/*/settings.php \
    --exclude=sites/all/* \
  ...
  rsync -avcz -e "ssh -l $REMOTE_USER" --delete --omit-dir-times --no-perms --no-times --no-group --chmod=ug=rwX \
    --exclude=sites/*/settings.php \
    --exclude=.svn --exclude=.git \
...
if [ "$LOCAL_IS_PRODUCTION_SERVER" -eq 1 ] ;then
  ...
  sudo rsync -avcz -e "ssh -l $REMOTE_USER" --delete --omit-dir-times --no-perms --no-times --no-group --chmod=ug=rwX  \
